...
(
	...
	coverfoto varbinary(max) not null,
...
VALUES 
	   /*Boeken*/
	   ('Ik zie je op het strand', 'Clemence ontmoet tijdens een vliegreis de aantrekkelijke, maar getrouwde Sam; als drie jaar later haar zus een nieuwe vriend heeft, blijkt dit Sam te zijn.',(select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\strand.png', single_blob) as boek1),'Uitgeverij Luitingh-Sijthoff', NULL, NULL, 'NL'),
	   ('Ragdoll', 'Rechercheur William Wolf Fawkes van de Metropolitan Police in Londen probeert met twee collega s de moordenaar te vinden die de resten van zijn slachtoffers tot een lappenpop aan elkaar naait.',(select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\ragdoll.jpg', single_blob) as boek2),'Uitgeverij Luitingh-Sijthoff', NULL, NULL, 'NL'),
	   ('Vind haar', 'Een rechercheur, werkzaam bij de politie in Boston, moet samen met haar team de zaak rond een getraumatiseerde jonge vrouw die voor de tweede maal gekidnapt is, als ook de vermissing van drie vrouwen oplossen.', (select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\Vindhaar.jpg', single_blob) as boek3),'Cargo', NULL, NULL, 'NL'),
	   ('Versplinterd', 'Rechercheurs Will Trent en Faith Mitchell maken jacht op de ontvoerder van een 17-jarig meisje dat na een bizarre moordpartij in haar eigen huis is gekidnapt.', (select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\Versplinterd.jpg', single_blob) as boek4),'HarperCollins', NULL, NULL, 'NL'),
	   ('Jager', 'Nadat de minister van Buitenlandse Zaken is vermoord, wordt Joona Linna overgehaald om weer voor de Zweedse veiligheidsdienst te gaan werken.', (select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\Jager.png', single_blob) as boek5),'Cargo', NULL, NULL, 'NL'),
	   ('Riskante relaties', 'Een Franse burggraaf die vrouwen naar zijn hand weet te zetten, stuit op een vrouw die weerstand biedt.', (select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\Riskante.png', single_blob) as boek6),'Uitgeverij De Arbeiderspers', NULL, NULL, 'NL'),
	   ('Sprakeloos', 'Familierelaas over de aftakeling van de moeder van de schrijver, wier taalvermogen na een beroerte is geminimaliseerd.', (select * from openrowset(bulk 'C:\Users\Afrem\Desktop\CASUS AD&SQL\images\Sprakeloos.png', single_blob) as boek7),'Prometheus', NULL, NULL, 'NL'),
